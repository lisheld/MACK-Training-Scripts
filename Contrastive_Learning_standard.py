...
parser.add_argument('--fineTune', action="store", dest="fineTune", default = 0, type=int)
...
if args.loss=='simclr':
    ...
    lossStr = '_VICReg'
modelName = "Contrastive_Learning%s"%lossStr
if args.label != "":
...
if args.fineTune:
    modelNameAdd = modelNameAdd + "_fineTune" + ("" if int(args.fineTune)<0 else str(args.fineTune))
...
fOne = h5py.File("/home/lsheldon/data/test_nom_bkg.z", 'r')
nom_bkg_data = fOne["taggerInputs"][:].astype(np.float32)
print('Loaded nom_bkg')
print(nom_bkg_data.shape)

fTwo = h5py.File("/home/lsheldon/data/test_nom_sig.z", 'r')
nom_sig_data = fTwo["taggerInputs"][:].astype(np.float32)
print('Loaded nom_sig')
print(nom_sig_data.shape)

fThree = h5py.File("/home/lsheldon/data/test_alt_bkg.z", 'r')
alt_bkg_data = fThree["taggerInputs"][:].astype(np.float32)
print('Loaded alt_bkg')
print(alt_bkg_data.shape)

fFour = h5py.File("/home/lsheldon/data/test_alt_sig.z", 'r')
alt_sig_data = fFour["taggerInputs"][:].astype(np.float32)
print('Loaded alt_sig')
print(alt_sig_data.shape)
...
nom_pts_train = nom_data[:nomTrainingDataLength,4]
alt_pts_train = alt_data[:altTrainingDataLength,4]
nom_pts_val = nom_data[nomTrainingDataLength:nomTrainingDataLength + nomValidationDataLength,4]
alt_pts_val = alt_data[altTrainingDataLength:altTrainingDataLength + altValidationDataLength,4]
nom_pts_test = nom_data[nomTrainingDataLength + nomValidationDataLength:,4]
alt_pts_test = alt_data[altTrainingDataLength + altValidationDataLength:,4]

nom_msd_train = nom_data[:nomTrainingDataLength,3]
alt_msd_train = alt_data[:altTrainingDataLength,3]
nom_msd_val = nom_data[nomTrainingDataLength:nomTrainingDataLength + nomValidationDataLength,3]
alt_msd_val = alt_data[altTrainingDataLength:altTrainingDataLength + altValidationDataLength,3]
nom_msd_test = nom_data[nomTrainingDataLength + nomValidationDataLength:,3]
alt_msd_test = alt_data[altTrainingDataLength + altValidationDataLength:,3]
...
nom_train_pt = nom_data[:nomTrainingDataLength,4]
alt_train_pt = alt_data[:altTrainingDataLength,4]
nom_val_pt = nom_data[nomTrainingDataLength:nomTrainingDataLength + nomValidationDataLength,4]
alt_val_pt = alt_data[altTrainingDataLength:altTrainingDataLength + altValidationDataLength,4]
nom_test_pt = nom_data[nomTrainingDataLength + nomValidationDataLength:,4]
alt_test_pt = alt_data[altTrainingDataLength + altValidationDataLength:,4]
    ...
    if args.EMD is not None or args.EMDNorm is not None:
        if args.EMD is not None:
            for i in range(particlesConsidered):
                ...
                nom_test_vars[:,i,0] = nom_test_vars[:,i,0]*nom_test_pt
                alt_test_vars[:,i,0] = alt_test_vars[:,i,0]*alt_test_pt
        train_pair_indices, train_labels = make_pairs_emd(nom_train_vars[:,:,:-1], alt_train_vars[:,:,:-1])
        val_pair_indices, val_labels = make_pairs_emd(nom_val_vars[:,:,:-1], alt_val_vars[:,:,:-1])
        test_pair_indices, test_labels = make_pairs_emd(nom_test_vars[:,:,:-1], alt_test_vars[:,:,:-1])
    else:
...
#these lines remove negative pairs
...
nom_test_data = np.zeros((test_pair_indices.shape[0],200))
alt_test_data = np.zeros((test_pair_indices.shape[0],200))
nom_train_data = np.zeros((train_pair_indices.shape[0],200))
alt_train_data = np.zeros((train_pair_indices.shape[0],200))
nom_val_data = np.zeros((val_pair_indices.shape[0],200))
alt_val_data = np.zeros((val_pair_indices.shape[0],200))
...
if args.trainFeat:
    ...
    def rotate_jet(jet):
        jet[:,:,1:-1] = rotate_via_numpy(jet[:,:,1:-1],np.random.uniform(-np.pi,np.pi,jet[:,0,0].shape))
        return jet
...
if args.trainClass:
    ...
    ### Splits data into training, test, and val
    nomTrainingDataLength = int(len(nom_data)*0.8)
    nomValidationDataLength = int(len(nom_data)*0.1)
    altTrainingDataLength = int(len(alt_data)*0.8)
    altValidationDataLength = int(len(alt_data)*0.1)
